...
app.post("/api/emailWriter", async (req: express.Request, res: express.Response) => {
  const { sender, recipient, propose } = req.body;
  const prompt = `Write an email from ${sender} to ${recipient} about ${propose} in 500 tokens`;
